...
1. **[הקדמה](#הקדמה)**
   - סקירה כללית של התוכנה SyncMe.
   - מאפיינים מרכזיים וחוויית המשתמש.

2. **[דרישות קדם](#דרישות-קדם)**
   - [דרישות צד שרת](#דרישות-צד-שרת)
   - [דרישות צד לקוח](#דרישות-צד-לקוח)

3. **[בחינת פתרונות אפשריים](#בחינת-פתרונות-אפשריים)**
   - ניתוח פתרונות קיימים.
   - "כיצד לממש את Dropbox" ו-"Rsync".

4. **[מבנה הפרויקט](#מבנה-הפרויקט)**
   - סקירה כללית של ארכיטקטורת הפרויקט.
   - הסבר על תהליך CI/CD.

5. **[הוראות שימוש](#הוראות-שימוש)**
   - [הקמת השרת](##הקמת-השרת)
     - [הפעלה מקומית](#הפעלה-מקומית)
     - [שימוש ב-Docker-compose](#שימוש-ב-docker-compose)
   - [הגדרת צד לקוח](#הגדרת-צד-לקוח)

6. **[ארכיטקטורת הפיתרון](#ארכיטקטורת-הפיתרון)**
   - [צד שרת](#צד-שרת)
   - [צד לקוח](#-צד-לקוח)

7. **[פרוקוטול](#פרוטוקול)**
8. **[סכמת זרימה](#סכמת-זרימה)**
9. **[בדיקות](#בדיקות)**
10. **[פערים](#פערים-במצב-הקיים)** 
...
מטרת מבני נתונים זה הוא לשמור את הקבצים. עבור כל לקוח במערכת. נוצרת תיקייה ובה נוצר קובץ zip שבתוכו לכל קובץ שהלקוח מעלה, שמם של הקבצים הוא ערך ה-id. מבני הנתונים תומך בקריאה ובכתיבה גם לחלקים של קובץ ולא לכולו.
#### Metadata Database
...
### צד לקוח
צד הלקוח הוא קצת יותר מורכב. משום שהוא צריך לשמור טבלה משלו עבור עץ התיקיות. נציג סרטוט סמכטי של אופן פעולתו. ואז נדון בהרחבה על כל רכיב
![image](https://github.com/guyStrauss/SyncMe/assets/11578138/6304f084-4893-4397-a8ed-4a3d11c1290b)

ה-Watcher קורא לDispatcher
#### Internal DB
מאחורי הקלעים, יש שימוש בספרייה `tinyDb` ספריית פייתון נהדרת לדברים כאלה. נציג את כל הפונקציות שמבנה הנתונים תומך בו
...
## פערים במצב הקיים
1. אם תוקף ישנה את user-id שלו בקוד. הוא יוכל לערוך קבצים של אנשים אחרים משום שהשרת לא נותן לך מזהה, אלא אתה מגנרץ בעצמך
2. משום שלא חוללתי certificate לשרת, המידע עובר מכווץ אך לא מוצפן
3. פיתרון הData Storage לא יעבוד עבור המון משתמשים, ויש בצורך בשירות כמו aws s3 bucket
חשוב לציין ששתי הבעיות הראשוניות קלות מאוד לפיתרון.
